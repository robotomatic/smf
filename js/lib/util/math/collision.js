...
Collider.prototype.collided = function() {
    return this.vertical.direction != "" || this.horizontal.direction != "" || this.depth.direction != "";
}

Collider.prototype.reset = function() {
    this.vertical.direction = "";
    this.vertical.amount = 0;
    this.vertical.x = null;
    this.vertical.y = null;
    this.vertical.z = null;
    
    this.horizontal.direction = "";
    this.horizontal.amount = 0;
    this.horizontal.x = null;
    this.horizontal.y = null;
    this.horizontal.z = null;
...
function collide(item1, item2, result) {
    ...
    result.horizontal.y = 0;
    result.horizontal.z = 0;
...
function collideRough(item1, item2, pad = 2) {
...
function collideFineVertical(item1, item2, result) {
    ...
    if (abs(vY) < hHeights) {
        ...
        if (oY) {
            ...
            result.vertical.x = item2.x;
            result.vertical.y = colY;
            result.vertical.z = item2.z;
        }
    }
    return result;
}
...
function collideFineHorizontal(item1, item2, result) {
    ...
    if (abs(vX) < hWidths) {
        ...
        if (oX) {
            ...
            result.horizontal.y = item2.y;
            result.horizontal.z = item2.z;
        }
    }
    return result;
}
...
function collideFineDepth(item1, item2, result) {
    ...
    if (abs(vZ) < hDepths) {
        ...
        if (oZ) {
            ...
            result.depth.x = item2.x;
            result.depth.y = item2.y;
            result.depth.z = colZ;
        }
    }
    return result;
