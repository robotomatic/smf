...
Item3D.prototype.renderItem3D = function(now, ctx, item, renderer, window, x, y, scale) {
    if (item.draw == false) return;
    var depth = item.depth * scale;
    if (item.parts) this.poly3d.renderItem3D(now, ctx, item, renderer, window, x, y, depth, scale);
    else this.rect3d.renderItem3D(now, ctx, item, renderer, window, x, y, depth, scale);
}