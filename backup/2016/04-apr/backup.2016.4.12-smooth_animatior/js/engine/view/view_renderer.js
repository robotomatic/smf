...
ViewRenderer.prototype.renderPlayers = function(now, ctx, stage, x, y, scale) {
    ...
    for (var i = 0; i < stage.players.players.length; i++) this.renderPlayerShadow(now, ctx, stage.players.players[i], x, y, scale);
    for (var i = 0; i < stage.players.players.length; i++) this.renderPlayer(now, ctx, stage.players.players[i], x, y, scale);
}


ViewRenderer.prototype.renderPlayerShadow = function(now, ctx, player, x, y, scale) {
    ...
    if (!collideRough(this.window, player.camerabox)) return;
    ...
    if (!player.lastX) player.lastX = px;
    var dx = (px - player.lastX) / 2;
    
    if (player.velX != 0) {
        if (dx < 0 && player.canMoveLeft || dx > 0 && player.canMoveRight) px = px - dx;    
    }
    
    if (!player.lastY) player.lastY = py;
    var dy = (py - player.lastY) / 2;
    if (player.velY != 0) {
        if (dy < 0 && player.canMoveUp || dy > 0 && player.canMoveDown) py = py - dy;
    } 
...
ViewRenderer.prototype.renderPlayer = function(now, ctx, player, x, y, scale) {
    ...
    player.draw(now, ctx, px, py, pw, ph, this.quality);            
}
