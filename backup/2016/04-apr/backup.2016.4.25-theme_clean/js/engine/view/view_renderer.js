...
ViewRenderer.prototype.renderLayer = function(now, stage, layer, ctx, x, y, width, height, scale) { 
    ...
    if (layer.cache === true) {
        ...
        if (layername == "players") this.renderPlayers(now, stage.players, ctx, x, y, scale);
...
ViewRenderer.prototype.renderPlayers = function(now, players, ctx, x, y, scale, quality) {
    if (!players || !players.players || !players.players.length) return; 
    players.render(now, this.window, ctx, x, y, scale, this.quality);
}
...
ViewRenderer.prototype.renderLayerItem = function(now, ctx, stage, layer, item, x, y, scale) {
    ...
    layer.drawItem(ctx, item, ix, iy, iw, ih, level.itemrendermanager, scale, cache, this.quality);
}
