...
function drawImage(ctx, image, x, y, width, height, dx, dy, dw, dh) { 
    if (dx) ctx.drawImage(image, x, y, width, height, dx, dy, dw, dh);
...
function drawSmoothRect(ctx, x, y, width, height) {
...
function drawDebug(ctx, item, x, y, scale) {
    ...
    for (var i = 0; i < parts.length; i++) {
        var part = parts[i];
        ctx.beginPath();
        drawShapeOutline(ctx, color, part, x + (part.x * scale), y + (part.y * scale), part.width * scale, part.height * scale, 2, 1, scale);
    }
}
