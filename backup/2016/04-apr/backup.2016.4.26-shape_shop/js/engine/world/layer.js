...
Layer.prototype.update = function(now, step) { 
    if (this.items) {
        for (var i = 0; i < this.items.length; i++) {
            this.updateItem(now, step, this.items[i]);
        }
    }
}

Layer.prototype.updateItem = function(now, step, item) { 
    if (item.actions) this.itemanimator.animate(now, step, item);
}


Layer.prototype.getItems = function() { return this.items; }

Layer.prototype.drawItem = function(ctx, item, x, y, width, height, renderer, scale, cache, quality) {
