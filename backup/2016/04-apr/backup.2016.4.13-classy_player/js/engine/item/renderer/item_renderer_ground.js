...
function ItemRendererGround() {
    ...
    // todo : clipping fucky --> make sure poly points are right, them replace with composite overlay?
...
ItemRendererGround.prototype.drawGround = function(ctx, polygon, x, y, width, height, scale, color, colordark, step) {
    ...
    ctx.save();
    ctx.beginPath();
    makeRoundPolygon(ctx, polygon.points, 20);
    ctx.closePath();
 
    // todo: need to show poly points. make sure that shit is in order.
