...
PlayerRenderer.prototype.draw = function(now, ctx, x, y, width, height, quality) {
    var dir = this.player.controller.getDirection(now);
    var state = this.player.controller.getState(now, dir);
    if (state == "idle") this.player.controller.idle(now);
    if (this.player.character) this.player.character.draw(now, ctx, this.player.color, x, y, width, height, dir, state, this.outline, this.debug, quality);
    else {
