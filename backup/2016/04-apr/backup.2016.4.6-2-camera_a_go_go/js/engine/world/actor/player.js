...
Player.prototype.isOnGround = function() { 
    ...
    return (this.groundpoint.y - (this.y + this.height) < .1);
}
...
//        ctx.globalAlpha = 1;
//        prect = new Rectangle(x, y, width, height);
//    }
    ...
    if (this.velX != 0) {
        if (dx < 0 && this.canMoveLeft || dx > 0 && this.canMoveRight) x = x - dx;    
    }
    this.lastX = x;
    ...
    if (this.velY != 0) {
        if (dy < 0 && this.canMoveUp || dy > 0 && this.canMoveDown) y = y - dy;
    } 
    this.lastY = y;
