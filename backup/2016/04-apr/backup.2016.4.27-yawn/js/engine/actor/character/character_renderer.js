...
CharacterRenderer.prototype.draw = function(ctx, color, mbr, puppet, outline, debug, quality) {
...
//    quality = 5;
    ...
    this.quality = (quality > this.quality) ? quality : this.quality;
    ...
    if (!this.canvas) {
        var cw = puppet.animbox.width * this.quality;
        var ch = puppet.animbox.height * this.quality;
        this.width = cw;
        this.height = ch;
    ...
    var rx = clamp(this.box.x - (((this.box.width * wpad) / 2) - (this.box.width / 2)));
    var ry = clamp(this.box.y - (this.box.height / 2));
    var rw = clamp(this.box.width * wpad);
    var rh = clamp(this.box.height * hpad);
...
//        var pw = clamp(this.outline.canvas.width - this.outline.weight);
//        var ph = clamp(this.outline.canvas.height - this.outline.weight);
