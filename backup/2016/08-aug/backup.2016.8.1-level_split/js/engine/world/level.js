...
Level.prototype.loadTheme = function(themename, theme) { 
    var rt = this.itemrendermanager.theme;
    if (!rt) {
        this.itemrendermanager.theme = new Theme(themename); 
        this.itemrendermanager.theme.background = theme.background;
        this.itemrendermanager.theme.items = JSON.parse(JSON.stringify(theme.items));
        return;
    }
...
Level.prototype.update = function(now, delta) {
    ...
    for (var i = 0; i < t; i++) this.layers[this.layerkeys[i]].update(now, delta);
}

Level.prototype.render = function(now, ctx, window, x, y, scale, quality, first, debug) {
    var t = this.layers.length;
    for (var i = 0; i < t; i++) this.layers[i].render(now, ctx, window, x, y, this.width, this.height, scale, quality, this.itemrendermanager, first);
    if (debug || this.debug) {
        for (var i = 0; i < t; i++) this.layers[i].debugLayer(now, ctx, window, x, y, this.width, this.height, scale, quality, this.itemrendermanager, first);
    }
}
