...
Layer.prototype.buildItems = function(renderer) { 
    ...
    layerbuilder.buildItems(this.items, renderer, this.depth, this.zindex);
}

Layer.prototype.isBackground = function() { 
    return (this.zindex && this.zindex < 500);
}
...
Layer.prototype.renderStart = function(now, ctx, window, x, y, width, height, scale, quality, renderer, first) {
    if (this.draw === false) return;
    this.items.renderStart(now, ctx, window, x, y, width, height, scale, quality, renderer, this.cache, this.blur, this.parallax, this.depth, this.top, first);
}
...
Layer.prototype.renderDebug = function(now, ctx, window, x, y, width, height, scale, quality, renderer, first) {
    if (this.draw === false) return;
...
Layer.prototype.renderEnd = function(now, ctx, window, x, y, width, height, scale, quality, renderer, first) {
    if (this.draw === false) return;
    this.items.renderEnd(now, ctx, window, x, y, width, height, scale, quality, renderer, this.cache, this.blur, this.parallax, this.depth, this.top, first);
}
