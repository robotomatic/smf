...
LayerBuilder.prototype.buildItems = function(items, renderer, depth, zindex) { 
    if (!items.items.length) return;
    var newitems = new Array();
    var t = items.items.length;
    for (var i = 0; i < t; i++) {
        this.buildItem(renderer, items.items[i], i, newitems, depth, zindex);
    }
    for (var ii = 0; ii < newitems.length; ii++) items.addItem(items.items.length, newitems[ii]);
    items.items.sort(sortByZIndex);
}


LayerBuilder.prototype.buildItem = function(renderer, item, index, newitems, depth, zindex) { 
    item.depth = depth;
    var theme = renderer.getItemTheme(item);
    if (!theme) return;
    var z = (item.zindex) ? item.zindex : 0;
    item.zindex = ((theme.zindex !== undefined) ? theme.zindex : zindex) + index + z;
