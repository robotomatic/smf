...
ItemRendererTheme.prototype.render = function(ctx, color, item, window, x, y, width, height, titem, scale) { 
    if (titem.parts) this.drawItemThemeParts(ctx, color, item, window, x, y, width, height, titem, scale);
    else this.drawItemThemePart(ctx, color, item, window, x, y, width, height, titem, scale, false);
}
...
ItemRendererTheme.prototype.drawItemThemeParts = function(ctx, color, item, window, x, y, width, height, titem, scale) { 
    ...
    for (var i = 0; i < t; i++) {
        ...
        this.drawItemThemePart(ctx, color, part, window, x, y, width, height, part, scale, true);
        if (part.parts && titem.itemtype != "group") {
            var c = this.getColor(ctx, part, part, x, y, width, height, scale, color);
            this.drawItemThemeParts(ctx, c, part, window, x, y, width, height, part, scale, true);
        }
    }
}

ItemRendererTheme.prototype.drawItemThemePart = function(ctx, color, part, window, x, y, width, height, titem, scale, relative) { 
    var c = this.getColor(ctx, part, titem, x, y, width, height, scale, color);
    this.drawItem(ctx, c, part, window, x, y, width, height, titem, scale, relative);
}


ItemRendererTheme.prototype.drawItem = function(ctx, color, item, window, x, y, width, height, titem, scale, relative) { 
    if (!item.width || !item.height) return;
    if (this.itemrendererdynamic.render(ctx, color, item, window, x, y, width, height, titem, scale)) return;
