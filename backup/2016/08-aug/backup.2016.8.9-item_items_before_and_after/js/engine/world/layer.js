...
Layer.prototype.updateItems = function(renderer) { 
    this.items.updateItems(this.depth, renderer);
}
...
Layer.prototype.renderAfter = function(now, ctx, window, x, y, width, height, scale, quality, renderer, first, players) {
    if (this.draw === false) return;
    this.items.renderAfter(now, ctx, window, x, y, width, height, scale, quality, renderer, this.cache, this.blur, this.parallax, this.depth, this.top, first, players);
}

Layer.prototype.renderColliders = function(now, ctx, window, x, y, width, height, scale, quality, renderer, first, players) {
    if (this.draw === false) return;
    this.items.renderColliders(now, ctx, window, x, y, width, height, scale, quality, renderer, first, players);
}
