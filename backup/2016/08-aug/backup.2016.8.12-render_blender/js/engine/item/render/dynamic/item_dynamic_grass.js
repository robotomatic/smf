...
ItemDynamicGrass.prototype.drawGrass = function(ctx, item, x, y, width, height, scale, titem, name) {
    ...
    if (color.gradient) {
        ...
        var it = spad + (ih / 2);
        if (it >= ih) it = ih / 2;
        var g = grassctx.createLinearGradient(0, it, 0, ih);
...
//        var it = ih / 2;
//        var gradient = color.gradient;
//        var g = grass.ctx.createLinearGradient(0, it, 0, ih);
//        var start = gradient.start;
//        var stop = gradient.stop;
//        g.addColorStop(0, start);
//        g.addColorStop(1, stop);
...
//    grass.ctx.fillStyle = color;
    ...
    for (var i = 0; i < blades.length; i++) {
        ...
        if (d < 0) {
            poly.points[1].x = round(poly.points[1].x - (amount * scale));
