...
Player.prototype.applyPhysics = function(physics, now) {
...
Player.prototype.updateLocation = function(when) {
...
//    if (this.move_left) this.velX -=acceleration;
//    else if (this.move_right) this.velX += acceleration;
    ...
    this.velX = (Math.abs(this.velX) < 1) ? 0 : round(this.velX);
    this.velY = round(this.velY);
    
    var delta = (when - this.lastwhen) / 15;
...
Player.prototype.draw = function(now, ctx, x, y, width, height, scale) {
    ...
    if (this.character) this.character.draw(now, ctx, this.color, x, y, width, height, dir, state, s, this.outline, this.debug);
    else {
