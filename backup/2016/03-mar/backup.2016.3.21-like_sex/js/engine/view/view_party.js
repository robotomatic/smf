...
PartyView.prototype.render = function(now, delta, stage) { 
    ...
    if (!this.view.render(now, delta, stage)) return;
    ...
    for (var i = 0; i < stage.level.layers.length; i++) this.renderLayer(now, delta, stage, stage.level.layers[i], canvas, ctx, mbr_x, mbr_y, mbr_w, mbr_h, scale, dx, dy); 
...
PartyView.prototype.renderLayer = function(now, delta, stage, layer, canvas, ctx, mbr_x, mbr_y, mbr_w, mbr_h, scale, dx, dy) { 
    ...
    if (layer.cache === true) {
        ...
        if (layername == "players") this.renderPlayers(now, delta, ctx, stage, this.transmbr, mbr_x, mbr_y, scale, dx, dy);
...
PartyView.prototype.renderPlayers = function(now, delta, ctx, stage, view, mbrx, mbry, scale, dx, dy) {
    
    // todo: move this shit into players class
    if (stage.players.drawshadows) {
        ctx.shadowBlur = .2;
        ctx.shadowColor = "black";
    }
    
    for (var i = 0; i < stage.players.players.length; i++) this.renderPlayer(now, delta, ctx, stage.players.players[i], view, mbrx, mbry, scale, dx, dy, i);
...
PartyView.prototype.renderPlayer = function(now, delta, ctx, player, view, mbrx, mbry, scale, dx, dy, index) {
