...
CharacterRenderer.prototype.draw = function(ctx, color, width, height, box, character, scale, debug) {
    ...
    if (opt) {
        ...
        this.newbox.x = box.width / 2;
        this.newbox.y = box.height / 2;
        this.newbox.width = box.width;
        this.newbox.height = box.height;
        this.ctx = ctx;
    }
    ...
    for (var i = 0; i < groups.length; i++) {
        ...
        if (!opt) poly.translate(box.x - (box.width / 2), box.y - (box.height / 2));
    ...
    if (this.debugrects && this.debugrects.length) this.drawDebugRectangles(this.debugrects, box, this.ctx, !opt);        
...
CharacterRenderer.prototype.drawDebugRectangles = function(rects, box, ctx, trans) { 
    if (rects.length) {
        for (var i = 0; i < rects.length; i++) {
            var r = rects[i];
            for (var ii = 0; ii < r.length; ii++) {
                if (trans) r[ii].translate(box.x - (box.width / 2), box.y - (box.height / 2));
