...
AnimationManager.prototype.animateCharacterState = function(character, partname, animation, lerp) {
    ...
    if (animation && animation.frames) {
        ...
        prevframe = (framenum > 0) ? animation.frames[framenum - 1] : animation.frames[animation.frames.length - 1];        
...
AnimationManager.prototype.applyAnimation = function(prevanimation, animation, part, t) {
    ...
    if (animation && part.height && part.width) {
        ...
        if (animation.height || animation.height == 0) part.height = lerp(part.height, part.height * (animation.height / 100), t);
        if (animation.width || animation.width == 0) part.width = lerp(part.width, part.width * (animation.width / 100), t);
