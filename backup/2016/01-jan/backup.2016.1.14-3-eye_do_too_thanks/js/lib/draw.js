...
function makePolygon(ctx, points) {
    ctx.moveTo(roundInt(points[0].x), roundInt(points[0].y));
    if (points[0].info && points[0].info == "round") ctx.moveTo(roundInt(points[1].x), roundInt(points[1].y));
    for (var i = 1; i < points.length; i++) {
        var p = points[i];
        if (p.info) {
            if (p.info == "round") {
            ...
            } else ctx.lineTo(roundInt(p.x), roundInt(p.y));
        } else ctx.lineTo(roundInt(p.x), roundInt(p.y));
    }
    
    if (points[0].info && points[0].info == "round") ctx.quadraticCurveTo(points[0].x, points[0].y, points[1].x, points[1].y);    
    else ctx.closePath();
}
