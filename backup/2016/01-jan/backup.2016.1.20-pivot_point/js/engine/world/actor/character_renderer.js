    ...
    if (this.groups.length) {
        ...
        for (var i = 0; i < this.groups.length; i++) {
            ...
            if (group.angle) {
                ...
                poly.setPoints(poly.rotate(group.rotation, group.center));
            }
...
CharacterRenderer.prototype.drawPart = function(box, partname, part, addpad, color) {
    ...
    if (part.angle) {
        ...
        if (part.parts) {
            this.rotateParts(group.angle, part.parts, cp);
        }
    } else if (part.rotation) {
        group.rotation = part.rotation;
        group.center = part.center;
    }
...
CharacterRenderer.prototype.rotateParts = function(angle, parts, cp) {

    var ks = Object.keys(parts);
    for (var i = 0; i < ks.length; i++) {
        var p = parts[ks[i]];
        
//        if (p.rotation) {
//            var rect = new Rectangle(p.x, p.y, p.width, p.height);
//            var points = rect.rotate(p.rotation, p.center);
//            p.x = points[0].x;
//            p.y = points[0].y;
//        }
        ...
        if (p.parts) this.rotateParts(angle, p.parts , cp);
