...
- figure out left arm 90 angle bullshit
- fix z order
    ...
    if (this.groups.length) {
        ...
        for (var i = 0; i < this.groups.length; i++) {
            ...
            if (group.angle) {
                ...
                if (group.parts) this.rotateGroup(ctx, this.groups, group);
            }
...
CharacterRenderer.prototype.rotateGroup = function(ctx, groups, group, parts) {
...
//            var rect = new Rectangle(group.center.x - 5, group.center.y - 5, 10, 10);
//            ppp.setPoints(rect.rotate(group.angle));

//            ppp.draw(ctx);
            

//            ppp.setPoints(groups[ii].points);
//            ppp.draw(ctx);
//
//            ctx.beginPath();
//            ctx.fillStyle = "pink";
             
            if (parts[keys[i]].parts) this.rotateGroup(ctx, groups, group, parts[keys[i]].parts);
