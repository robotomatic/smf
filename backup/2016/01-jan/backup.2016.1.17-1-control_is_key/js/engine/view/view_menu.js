function MenuView(id, width, height, scale) {
    ...
    this.view.canvas_render.onclick = function() {
        ...
        controller.camera.shakeScreen(8, 800);
...
MenuView.prototype.update = function(now, delta, stage) {
    ...
    for (var i = 0; i < stage.players.length; i++) {
        ...
        if (stage.players[i].x < 20 && stage.players[i].move_left) {
            stage.players[i].left(false);
        }
        else if (stage.players[i].x > stage.level.width - (stage.players[i].width + 20) && stage.players[i].move_right) {
            stage.players[i].right(false);
        }
        ...
        if (!stage.players[i].paused && !stage.players[i].move_left && !stage.players[i].move_right) {
            ...
            if (!stage.players[i].look_straight && !stage.players[i].look_left && !stage.players[i].look_right) {
                var dir = random(0, 2);
                if (dir == 0) {
                    stage.players[i].lookLeft(true);
                    stage.players[i].timeout("lookLeft", false, random(2000, 3000));
                } else if (dir == 1) {
                    stage.players[i].lookRight(true);
                    stage.players[i].timeout("lookRight", false, random(2000, 3000));
                } else {
                    stage.players[i].lookStraight(true);
                    stage.players[i].timeout("lookStraight", false, random(3000, 5000));
                }
            }
        }
