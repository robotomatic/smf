...
ItemRenderer.prototype.render = function(ctx, color, renderer, item, window, x, y, width, height, titem, scale, drawdetails) {
    var r = this.renderers[renderer];
    if (r) {
        r.draw(ctx, color, item, window, x, y, width, height, titem, scale, drawdetails);
        return true;
    }
    return false;
}
