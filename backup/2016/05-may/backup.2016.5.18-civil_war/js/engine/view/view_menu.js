...
MenuView.prototype.updateNPC = function(stage, npc) {
    ...
    if (player.controller.x < 20 && player.controller.move_left) player.controller.left(false);
    else if (player.controller.x > stage.level.width - (player.controller.width + 20) && player.controller.move_right) player.controller.right(false);
    if (!player.controller.paused && !player.controller.move_left && !player.controller.move_right && player.controller.grounded) {
        var dir = random(0, 8);
        if (dir == 0) {
            if (player.controller.x > 20) {
                npc.doActionTimeout("left", true, false, random(1000, 3000));
            }
        } else if (dir == 1) {
            if (player.controller.x < stage.level.width - (player.controller.width + 20))  {
