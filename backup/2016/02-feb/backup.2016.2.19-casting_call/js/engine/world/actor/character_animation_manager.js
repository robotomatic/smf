...
CharacterAnimationManager.prototype.applyAnimationSize = function(prevanimation, animation, part, t) {
    if (animation.height || animation.height == 0) {
        ...
        if (animation.height > ph.height) part.height = lerp(partheight, partheight * (animation.height / 100), t);
        else part.height = lerp(partheight * (ph / 100), partheight * (animation.height / 100), t);
    }
    if (animation.width || animation.width == 0) {
        var pw = prevanimation ? (prevanimation.width || 0) : 0;
        var partwidth = part.width;
        if (animation.width > pw.width) part.width = lerp(partwidth, partwidth * (animation.width / 100), t);
        else part.width = lerp(partwidth * (pw / 100), partwidth * (animation.width / 100), t);
    }
    return part;
}
...
CharacterAnimationManager.prototype.applyAnimationPosition = function(prevanimation, animation, part, t) {
    if (animation.x || animation.x == 0) {
        ...
        if (animation.x >= px || animation.x < 0) {
            if (px < 0)  part.x = lerp(partx + px, partx + animation.x, t);                    
            else if (animation.x == px) part.x = lerp(partx + px, partx + animation.x, t);                    
            else part.x = lerp(partx, partx + animation.x, t);                    
        } else {
            part.x = lerp(partx + px, partx - animation.x, t);                    
        }
    }
    if (animation.y || animation.y == 0) {
        ...
        if (animation.y >= py || animation.y < 0) {
            if (py < 0) part.y = lerp(party + py, party + animation.y, t);                    
            else if (animation.y == py) part.y = lerp(party + py, party + animation.y, t);                    
            else part.y = lerp(party, party + animation.y, t);                    
        } else {
            part.y = lerp(party + py, party - animation.y, t);                    
        }
    }
    return part;
}
