...
//    if (item.parts) {
//        for (let part in item.parts) this.updateItem(step, item.parts[part]);
//    }
}
...
Layer.prototype.collideItemParts = function(player, item) {
    ...
    for (let i = 0 ; i < item.parts.length; i++) {
        ...
        let col = this.collideItemPart(player, item, item.parts[i]);
        if (col) out = true;
    }
    return out;
}
...
Layer.prototype.collideItemPart = function(player, item, part) {
    ...
    let px = item.x + part.x;
    let py = item.y + part.y;
