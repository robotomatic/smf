...
//    if (this.particles.length) for(let i = 0; i < this.max; i++) this.particles[i].translate(dx, dy);        
...
ParticleEmitter.prototype.update = function(x, y, scale, ctx) {
    ...
    let size = this.size + Math.random() * 1;

    if (!this.particles.length) for(let i = 0; i < this.max; i++) this.particles.push(new Particle(x, y, size));    
...
//        p.radius *= Math.random() * 1;
        ...
        if (p.radius > .1) {
            ...
            let gradient = ctx.createRadialGradient(p.location.x, p.location.y, 0, p.location.x, p.location.y, p.radius * scale);
