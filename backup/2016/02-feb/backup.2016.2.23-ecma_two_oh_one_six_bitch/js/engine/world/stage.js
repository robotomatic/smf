...
Stage.prototype.setViews = function(views) { 
    this.views = views;
    if (this.views) for (let i = 0; i < this.views.length; i++) this.views[i].setLevel(this.level);
    return this.views; 
}
...
Stage.prototype.updatePlayers = function(now, step, players) { 
    if (this.players) for (let i = 0; i < this.players.length; i++) this.updatePlayer(now, step, this.players[i]);
}
...
Stage.prototype.updateViews = function(now, dt) { if (this.views) for (let i = 0; i < this.views.length; i++) this.views[i].update(now, dt, this); }

Stage.prototype.render = function(now, dt) { if (this.views) for (let i = 0; i < this.views.length; i++) this.views[i].render(now, dt, this); }

Stage.prototype.setMessage = function(message) { if (this.views) for (let i = 0; i < this.views.length; i++) this.views[i].setMessage(message); }
