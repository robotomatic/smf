...
//    let ctx = c.context;
    ...
    let view = getViewWindow(canvas, this.view.width, this.view.height);
    
    let cx = this.view.width / 2;
    let cy = (this.view.height / 2);
...
//    }
    ...
    let ptrans = translateItem(canvas.width, canvas.height, pbox, this.view.width, this.view.height);
    let x = this.menubox.x;
    let y = this.menubox.y;
    let pw = this.menubox.width * this.zoom;
    let ph = this.menubox.height * this.zoom;
    let px = x * this.zoom;
    let py = y * this.zoom;
    let dx = cx - (px + (pw / 2) );
    let dy = cy - (py + (ph / 2) );
    ...
    let bounds_rel = translateRelative(bounds, dx, dy, this.zoom);
    let bounds_trans = translateItem(canvas.width, canvas.height, bounds_rel, this.view.width, this.view.height);
    let bounds_bottom_y = bounds_trans.y + bounds_trans.height;
    let border_height = (canvas.height - view.height)  / 2;
    let border_top = view.height + border_height;
    let height_diff = border_top - bounds_bottom_y;
    let offset_y = (height_diff > 0) ? height_diff : 0;
...
//    if (players) for (let i = 0; i < players.length; i++) {
...
CharactersView.prototype.renderPlayer = function(now, delta, canvas, ctx, player, dx, dy, offset_y) {
    let rel = translateRelative(player, dx, dy, this.zoom);
    let rtrans = translateItem(canvas.width, canvas.height, rel, this.view.width, this.view.height);
