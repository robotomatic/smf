...
MenuView.prototype.update = function(now, delta, stage) {
    ...
    for (let i = 0; i < stage.players.length; i++) {
        ...
        if (!stage.players[i].paused && !stage.players[i].move_left && !stage.players[i].move_right && stage.players[i].grounded) {
            let dir = random(0, 8);
            if (dir == 0) {
                ...
                if (!stage.players[i].falling) {
                    let safe = true;
                    for (let ii = 0; ii < stage.players.length; ii++) {
                        if (ii == i) continue;
                        if (stage.players[ii].paused) {
                            let d = Math.abs(stage.players[ii].x - stage.players[i].x);
                            if (d < 20) {
                                safe = false;
                                break;
                            }
                    ...
                    if (safe) {
                        ...
                        if (dir2 == 0) {
                            if (stage.players[i].x > 20) {
                                stage.players[i].lookStop();
                                stage.players[i].left(true);
                                stage.players[i].timeout("left", false, random(1000, 2000));
                            }
                        } else if (dir2 == 1) {
                            if (stage.players[i].x < stage.level.width - (stage.players[i].width + 20)) {
                                stage.players[i].lookStop();
                                stage.players[i].right(true);
                                stage.players[i].timeout("right", false, random(1000, 2000));
                            }
                        }
                    }
                }
...
//    let ctx = this.view.ctx_buffer;
    let canvas = this.view.canvas_render;
    let ctx = this.view.ctx_render;
    
    let view = getViewWindow(canvas, stage.level.width, stage.level.height );
    let dx = (canvas.width - view.width) / 2;                 
    let dy = (canvas.height - view.height) - 100;               
    
    let centerpoint = new Point(dx, dy);
    let center = this.camera.getCenterPoint(now, centerpoint);
    dx += center.x - dx;
    dy += center.y - dy;
    
    this.renderPlayers(now, delta, ctx, stage, view, stage.level.width, stage.level.height, dx, dy); 
}

MenuView.prototype.renderPlayers = function(now, delta, ctx, stage, view, width, height, dx, dy) {
    for (let i = 0; i < stage.players.length; i++) this.renderPlayer(now, delta, ctx, stage.players[i], view, width, height, dx, dy);
}

MenuView.prototype.renderPlayer = function(now, delta, ctx, player, view, width, height, dx, dy) {
    
    let trans = new Rectangle(player.x, player.y, player.width, player.height);
    trans = translateItem(view.width, view.height, trans, width, height);

    let itemx = trans.x + dx;
    let itemy = trans.y + dy;
    
    let scale = (trans.width) ? trans.width / player.width : trans.height / player.height ;
