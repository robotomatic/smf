...
function View(id, width, height, scale, canvasclass) {
    ...
    this.last = timestamp();
    this.alpha = 0;
    let uie = document.getElementById(this.id + "-text");
    if (uie) this.ui = new UI(uie);
}

View.prototype.removeCanvases = function() { 
    let canvas = this.parent.getElementsByTagName("canvas");
    for(let i = 0; i < canvas.length; i++) this.parent.removeChild(canvas[i]);
}

View.prototype.setLevel = function(level) { 
    if (level.layers && level.layers.length) for (let i = 0; i < level.layers.length; i++) this.addLayer(level.layers[i]); 
}
...
//        
//        // todo: already have them other canvii...
...
View.prototype.resizeUI = function() { 
    ...
    if (this.ui) {
        let view = getViewWindow(this.canvas_render, this.width, this.height);
        let bw = ((this.canvas_render.offsetWidth) - view.width) / 2;
        let bh = (this.canvas_render.offsetHeight - view.height) / 2;
        let pad = 15;
...
View.prototype.resizeText = function() {
    ...
    if (this.ui) {
        let ui = this.ui.getUI();
        let p = ui.parentNode;
        let pw = ui.offsetWidth;
        let fs = Number(pw / 30);
        if (fs > 15) fs = 15;
...
View.prototype.render = function(now, delta, stage) {
//    if (!this.dorender) {
