...
function copyArray(a) {
    ...
    for (let i = 0; i < keys.length; i++) out[keys[i]] = a[keys[i]];
    return out;
}
...
function translateItem(winwidth, winheight, box, width, height) {
    ...
    let dx = (winwidth - (width * d)) / 2;
    let dy = (winheight - (height * d)) / 2;
    let ox = (box.x + (dx / d)) * d;
    let oy = (box.y + (dy / d)) * d;
    
//    let out = new Rectangle(ox, oy, box.width * d, box.height * d);
//    return out;
...
function translateRelative(item, dx, dy, zoom) {
    zoom = (zoom) ? zoom : 1;
    let r = new Rectangle((item.x * zoom) + dx, (item.y * zoom) + dy, item.width * zoom, item.height * zoom);
    return r;
}
...
function getMbr(items, pad, box) {
    ...
    for (let i = 0; i < items.length; i++) {
        let item = (items[i].camerabox) ? items[i].camerabox : items[i];
...
function fadeToColor(rgbColor1, rgbColor2, ratio) {
    if (!rgbColor1) return rgbColor2;
    let color1 = rgbColor1.substring(4, rgbColor1.length - 1).split(','),
