...
function resizeElements() {
    if (document.getElementsByClassName("auto").length) {
        let elem = document.getElementsByClassName("auto");
    ...
    if (document.getElementsByClassName("fit-height").length) {
        let elem = document.getElementsByClassName("fit-height");
        for (let i = 0; i < elem.length; i++) {
            elem[i].style.height = window.innerHeight + "px";
        }
    }
    if (document.getElementsByClassName("ui-height").length) {
        let elem = document.getElementsByClassName("ui-height");
        for (let i = 0; i < elem.length; i++) {
            let uih = elem[i].getAttribute("ui-height");
            if (uih) {
                uih -= uih *.02;
                let nh = window.innerHeight * (uih / 100); 
                elem[i].style.height = nh + "px";
            }
        }
    }
    if (document.getElementsByClassName("ui-top").length) {
        let elem = document.getElementsByClassName("ui-top");
        for (let i = 0; i < elem.length; i++) {
            let tp = elem[i].getAttribute("ui-top");
            if (tp) {
                let p = findParent(elem[i], "ui-top-parent");
                if (!p) elem[i].parentNode;
                let pt = p.offsetHeight;
                let nt = pt * (tp / 100); 
