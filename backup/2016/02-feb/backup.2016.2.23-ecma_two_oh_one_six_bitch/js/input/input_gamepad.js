...
GamepadInput.prototype.udpatePad = function(now, delta, pad, index) {
    ...
    for (let i = 0; i < this.bindings[index].length; i++) {
        let binding = this.bindings[index][i].binding;
        let binditem = binding.item;
        let bindelement = binding.element;
        
        if (!pad[binditem] || !pad[binditem][bindelement]) continue;
        let paditem = pad[binditem][bindelement];
        
        let bindtest = binding.test;
        let padval = this[bindtest](paditem, binding.value);
        
        let bindaction = binding.action;
        let bindobj = this.bindings[index][i].obj;

        if (binding.live) {
            let actionname = binditem + "_" + bindelement + "_" + bindaction;
...
GamepadInput.prototype.bindGamepads = function(obj, bindings) { 
    ...
    let pads = navigator.getGamepads();
    for (let i = 0; i < pads.length; i++) if (pads[i]) this.bind(obj, i, pads[i].id, bindings);
} 

GamepadInput.prototype.bind = function(obj, index, padid, bindings) { 
    this.bindings[index] = new Array();
    for (let i = 0; i < bindings.bindings.length; i++) this.mapButton(obj, index, padid, bindings.bindings[i]); 
}
