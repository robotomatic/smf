...
Players.prototype.getMbr = function(pad, mbr) {
    ...
    for (var i = 0; i < this.players.length; i++) {
        ...
        var item = (this.players[i].camera.camerabox) ? this.players[i].camera.camerabox : this.players[i].controller;
...
Players.prototype.smoothPlayer = function(now, player, scale) {
    ...
    player.controller.lastY = player.controller.y;
    ...
    if (player.controller.velX != 0) {
        if (dx < 0 && player.controller.canMoveLeft || dx > 0 && player.controller.canMoveRight) {
            px = px - dx;    
        }
    }
    ...
    if (player.controller.velY != 0) {
        if (dy < 0 && player.controller.canMoveUp || dy > 0 && player.controller.canMoveDown) {
            py = py - dy;
        }
    } 
