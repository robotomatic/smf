...
Layer.prototype.collideItem = function(player, item) {
    ...
    // todo: need to use player delta [x, y] vector to test collisions.
    
    
    
    if (item.action) {
        this.box.x = item.lastX;
        this.box.y = item.lastY;
    } else {
        this.box.x = item.x
        this.box.y = item.y;
    }
...
//        c = player.collideWith(this.box, c);
        ...
        var xx = ix - player.controller.x;
        var yy = iy - player.controller.y;
