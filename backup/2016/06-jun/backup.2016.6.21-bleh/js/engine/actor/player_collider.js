...
PlayerCollider.prototype.handleCollideActionHorizontal = function(player, collider, result) {
    if (Math.abs(player.controller.velX) < 1) {
        if (!player.controller.dx) {
            player.controller.dx = player.controller.x - collider.x;
        }
        player.controller.x = round(collider.x + player.controller.dx);
