...
Item3D.prototype.renderItemParts3D = function(now, ctx, window, x, y, item, parts, width, height, scale, quality, renderer, depth, top) {
    ...
    this.drawItemPartsPolygon3D(ctx, this.polygon, {"sides" : sidecolor, "top" : topcolor, "bottom" : bottomcolor}, depth, window.width, window.height, scale, x, y, window, true, false);

    
    var topsidecolor = theme.topsidecolor;
    var toptopcolor = theme.toptopcolor;
    var topbottomcolor = theme.topbottomcolor;
...
Item3D.prototype.drawItemPartsPolygon3D = function(ctx, poly, colors, depth, width, height, scale, x, y, window, drawbottom, drawtop) {
    ...
    for (var i = 1; i < t; i++) {
        ...
        // todo: - check for ramp
        var horiz = Math.abs(p1.y - p2.y) < 10;
        var vert = Math.abs(p1.x - p2.x) < 10;
        
        var ramptopleft = p1.x < p2.x && p1.y > p2.y;
        var ramptopright = p1.x < p2.x && p1.y < p2.y;
        var ramptop = ramptopleft || ramptopright;
        
        var rampbottomleft = p1.x > p2.x && p1.y > p2.y;
        var rampbottomright = p1.x > p2.x && p1.y < p2.y;
        var rampbottom = rampbottomleft || rampbottomright;

        var top = horiz && (p1.x < p2.x);
        var bottom = horiz && !top;
        var left = vert && (p1.y > p2.y);
        var right = vert && !left;
        
        if (!drawtop && (top)) continue;
        if (!drawbottom && (bottom)) continue;
    ...
    this.drawPolygonTops3D(ctx, tops, colortop);
}




Item3D.prototype.drawPolygonTops3D = function(ctx, tops, color) {
    ctx.fillStyle = color;
...
//    np.x = p1.x + d * Math.cos(a);
//    np.y = p1.y + d * Math.sin(a);
//    
//    var dt = np.y >= cp.y;
//    var dl = np.x > cp.x;
...
//        var debug = true;
//        if (dt) {
//            if (rampbottomleft && dl) return true;
//            else if (rampbottomright && !dl) return true;
//            else return false;
//        }
//    }
