...
Item3D.prototype.renderItemRect3D = function(now, ctx, window, x, y, item, width, height, scale, quality, renderer, depth, top, parallax) {
    ...
    var iy = 0;
    if (item.action) {
        ix = (item.lastX ? item.lastX : item.x) - x;
        iy = (item.lastY ? item.lastY : item.y) - y;
    } else {
        ix = item.x - x;
        iy = item.y - y;
    }
    ...
    this.p2.x = (item.x + item.width - x) * scale;
    this.p2.y = this.p1.y;
    if (parallax) this.p2.x += dcx * parallax;
    
    this.p3.x = this.p2.x;
    this.p3.y = (item.y + item.height - y) * scale;
    ...
    if (this.shouldProject(this.p3, this.p4, scale, x, y, window, ctx)) {
...
Item3D.prototype.drawItemPartsPolygon3D = function(ctx, poly, colors, depth, width, height, scale, x, y, window, drawbottom, drawtop) {
    ...
    for (var i = 1; i < t; i++) {
        ...
        if (!this.shouldProject(this.p1, this.p2, scale, x, y, window, ctx)) continue;
        
        var horiz = Math.abs(this.p1.y - this.p2.y) < 10;
        var vert = Math.abs(this.p1.x - this.p2.x) < 10;
