...
Items.prototype.render = function(now, ctx, window, x, y, width, height, scale, quality, renderer, cache, blur, parallax, depth, top, first) {
    ...
    for (var i = 0; i < t; i++) {
        ...
        if (depth) this.item3D.renderItem3D(now, ctx, window, x, y, item, width, height, scale, quality, renderer, depth, top, parallax);
        if (!cache || !this.itemcache.cacheItem(ctx, item, window, renderer, scale, this.drawdetails, quality, blur, parallax)) {
            item.render(now, ctx, window, x, y, scale, renderer, this.drawdetails);
        }
