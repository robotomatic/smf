...
Players.prototype.render = function(now, window, ctx, x, y, scale, quality) {
    ...
    // todo: need to see what we're standing on and project if necessary
//    y += 2;
...
Players.prototype.smoothPlayers = function(now, scale) {
    for (var i = 0; i < this.players.length; i++) this.players[i].smooth(now, scale);
}
...
//    var ppx = round(player.controller.x - x);
//    
//    if (player.controller.velX || player.controller.dx) {
//        console.log(this.lastpx + " - " + this.lastx + " == " + this.lastppx);
//        console.log(player.controller.x + " - " + x + " == " + ppx);
//        console.log("-----------------------------------------");
...
//    }
//    this.lastppx = ppx;
//    this.lastpx = player.controller.x;
//    this.lastx = x;
    
    
    var depth = false;
    var t = player.collider.levelcollisions.length;
    for (var i = 0; i < t; i++) {
        var c = player.collider.levelcollisions[i];
        if (c.direction != "b") continue;
        if (c.depth) {
            depth = true;
            break;
        }
