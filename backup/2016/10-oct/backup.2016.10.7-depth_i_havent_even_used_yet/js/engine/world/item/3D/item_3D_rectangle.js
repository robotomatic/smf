...
Item3DRectangle.prototype.renderItem3D = function(now, ctx, item, renderer, window, x, y, depth, scale, floodlevel) {
    ...
    depth *= item.scalefactor;
    
    if (floodlevel && iy + ih > floodlevel) {
        var d = (iy + ih) - floodlevel;
        ih -= d;
    }
    ...
    if (shouldProject(this.p2, this.p3, scale, x, y, wc, this.cp)) {
        ...
        if (outline) this.polygon.drawOutline(ctx, this.sidecolor, 1 * scale);
    }
    ...
    if (item.top !== false) {
        if (shouldProject(this.p1, this.p2, scale, x, y, wc, this.cp)) {
            ...
            ctx.beginPath();
            this.polygon.draw(ctx);
            if (outline) this.polygon.drawOutline(ctx, this.topcolor, 1 * scale);
        }
    }
    ...
    if (shouldProject(this.p3, this.p4, scale, x, y, wc, this.cp)) {
        ...
        ctx.beginPath();
        this.polygon.draw(ctx);
        if (outline) this.polygon.drawOutline(ctx, this.bottomcolor, 1 * scale);
    }
}