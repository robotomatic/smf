...
Item.prototype.isVisible = function(window, wmbr, pad = 0) {
    ...
    if (this.width != "100%") {
        if (mbr.x > (window.x + window.width + pad)) return false;
        if ((mbr.x + mbr.width) < window.x - pad) return false;
    }
    
    if (this.height != "100%") {
        if (mbr.y > (window.y + window.height + pad)) return false;
        if ((mbr.y + mbr.height) < window.y - pad) return false;
    }

    if (this.depth != "100%") {
        if (mbr.z + mbr.depth < window.z - pad) return false;
    }
...
Item.prototype.render = function(now, ctx, window, x, y, scale, renderer) {
    ...
    if (renderer) renderer.drawItem(ctx, this.color, this, window, x, y, scale);
        ...
        ctx.fillStyle = item.color ? item.color : "green"; 
        this.box.draw(ctx);
    }
}
