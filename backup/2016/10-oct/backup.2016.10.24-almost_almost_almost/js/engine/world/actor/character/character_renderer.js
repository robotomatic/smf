...
CharacterRenderer.prototype.draw = function(ctx, color, mbr, puppet, outline, debug, quality, charname, floodlevel) {
...
//    quality = 5;
    ...
    if (quality) {
        if (this.quality != quality) this.canvas = null;
        this.quality = quality;
    } else quality = this.quality;
    
    this.outline.draw = outline;
    this.outline.drawinner = outline;
    
    var wpad = 4;
    var hpad = 2;
    
    if (!this.canvas) {
        var cw = puppet.animbox.width * quality;
        var ch = puppet.animbox.height * quality;
        this.width = cw;
        this.height = ch;
    ...
    if (this.debugrects && this.debugrects.length) this.drawDebugRectangles(this.debugrects, this.box, this.ctx);        

    var rx = this.box.x - (((this.box.width * wpad) / 2) - (this.box.width / 2));
    var ry = this.box.y - (this.box.height / 2);
    var rw = this.box.width * wpad;
    var rh = this.box.height * hpad;
    ...
    if (this.outline.draw) {
        ...
        this.image.x = 0;
        this.image.y = 0;
        this.image.width = this.outline.canvas.width;
        this.image.height = this.outline.canvas.height;
        this.image.data = this.canvas;
        this.image.draw(this.outline.ctx);
        ...
        this.image.y = pp;
        this.image.width = pw;
        this.image.height = ph;
        this.image.data = this.outline.canvas;
        this.image.draw(ctx, rx, ry, rw, rh);
    }
    ...
    var nby = this.newbox.y;
    var nbh = this.newbox.height;

    if (floodlevel) {
        if (this.box.y <= floodlevel) {
            var ph = this.box.y + this.box.height;
            if (ph > floodlevel) {
                var d = ph - floodlevel;
                var dh = nbh - d;
//                console.log(ph + "  --> " + floodlevel + "  ::  " + nbh + "  -  " + d + "  ==  " + dh);
                var ch = nby + dh;
                this.image.clipY(this.image.height - ch);
            }
        } else {
//            console.log("Avast...");
            return;
...
CharacterRenderer.prototype.drawInnerOutline = function( ) { 
    ...
    this.outline.ctx.fillStyle = this.outline.color;
    var r = new Rectangle(0, 0, this.outline.canvas.width, this.outline.canvas.height);
    this.outline.ctx.beginPath();
    r.draw(this.outline.ctx);
    this.outline.ctx.save();
    this.outline.ctx.globalCompositeOperation = 'destination-in';
    ...
    this.image.width = this.outline.canvas.width;
    this.image.height = this.outline.canvas.height;
    this.image.data = this.tempcanvas;
    this.image.draw(this.outline.ctx);
