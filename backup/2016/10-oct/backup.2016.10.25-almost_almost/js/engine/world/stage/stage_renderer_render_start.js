...
StageRendererStart.prototype.getAllItemsPlayers = function(mbr, window, graphics, stage) {
    ...
    if (players && players.players) {
        ...
        for (var pi = 0; pi < pt; pi++) {
            ...
            this.renderitems.centerbottom = rp.controller.groundpoint.y - 1;
        }
    }
}
...
StageRendererStart.prototype.sortItemsVertical = function(window) {
    ...
    this.sortItemsVerticalCenter(this.renderitems.center, cp);
}
...
StageRendererStart.prototype.sortItemsVerticalCenter = function(items, cp) {
    items.z = new Array();
    var t = items.all.length;
    for (var i = 0; i < t; i++) {
        var item = items.all[i];
    ...
    this.sortItemsVerticalYZ(items.z, cp);
}
...
StageRendererStart.prototype.sortItemsVerticalYZ = function(items, cp) {
    ...
    for (var ii = 0; ii < tt; ii++) {
        ...
        itemitems.x = {};
        this.sortItemsVerticalYZHorizontal(itemitems, itemitems.x, cp)
    }
}
...
StageRendererStart.prototype.sortItemsVerticalYZHorizontal = function(items, newitems, cp) {
    newitems.left = new Array();
    newitems.center = new Array();
    newitems.right = new Array();
    var keys = Object.keys(items);
    var t = keys.length;
    for (var i = 0; i < t; i++) {
        var key = keys[i];
        ...
        newitems[side][newitems[side].length] = item;
    }
}