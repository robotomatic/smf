function View(id, width, height, devtools) {
    if (!id) return;
    this.id = id;
    this.width = width;
    this.height = height;
    ...
    this.gamewindow = document.getElementsByClassName("game-window")[0];
    
    var uie = document.getElementById(this.id + "-text");
    if (uie) this.ui = new UI(uie);
    if (devtools) {
        this.devtools = new DevTools(devtools);
        var main = document.getElementById("main");
        var d = document.createElement("div");
        d.innerHTML = this.devtools.getUI();
        main.appendChild(d);
        var dt = document.getElementById("dev-ui");
        var newid = "dev-ui-" + this.id;
        dt.id = newid;
        this.devtools.setID(newid);
        this.devtools.initializeUI();
    }
}
...
View.prototype.resizeUI = function() { 
    if (this.ui) {
        ...
        this.resizeDevUI(view);
        this.resizeText();
    }
}

View.prototype.resizeDevUI = function(view) {
    if (this.devtools) {
        var dw = document.body.clientWidth;
        var rect = this.canvas_render.getBoundingClientRect();
        var left = rect.left;
        var width = this.canvas_render.offsetWidth;
        var right = dw - (left + width);
        
        var top =  rect.top;
        
        if (this.gamewindow) {
            var winrect = this.gamewindow.getBoundingClientRect();
            top = winrect.bottom - 60;
            right = dw - winrect.right + 10;
        }
        
        
        this.devtools.setLocation(right, top);
    }
}
