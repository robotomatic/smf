...
MenuView.prototype.removeCanvases = function() {
    var canvas = this.parent.getElementsByTagName("canvas");
    for(var i = 0; i < canvas.length; i++) {
        this.parent.removeChild(canvas[i]);
    }    
}
...
MenuView.prototype.resizeText = function() {
...
MenuView.prototype.updateUI = function(stage) {
    if (this.aplha < 0) return;
    if (!this.uiready) {
        this.ui = new UI(document.getElementById("menu-ui"));
        this.resizeUI();
        fadeIn(this.ui.getUI());
        this.uiready = true;
    }
    if (stage.npcs) if (stage.npcs.npcs[stage.npcs.currentnpc]) this.ui.setMessage(stage.npcs.npcs[stage.npcs.currentnpc].player.name);;
}