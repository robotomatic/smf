function View(id, width, height, scale, roundcorners) {
    if (!id) return;
    this.id = id;
    this.width = width;
    this.height = height;
    ...
    this.canvas_render.className = "fade-hide game-canvas";
    this.canvas_render.className += " absolute";
    if (roundcorners == true) this.canvas_render.className += " round-corners"
    this.scale = scale;
...
View.prototype.resize = function() {
    ...
    for (var layername in this.canvas) this.sizeCanvas(this.canvas[layername].canvas, cleft, ctop, cwidth, cheight);
...
View.prototype.resizeUI = function() { 
    if (this.ui) {
        ...
        var pad = 15;
        this.ui.setLocation(view.x +bw + pad, view.y +bh + pad);
        this.resizeText();
    }
}
