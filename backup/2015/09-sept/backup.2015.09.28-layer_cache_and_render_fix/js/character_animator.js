...
CharacterAnimator.prototype.animate = function(box, state, direction, character) {
    ...
    /*
    var out;
    for (var a in this.animations) out = this.animateAnimation(a, this.animations[a], animbox, state, direction, animchar);
    return out;
}

CharacterAnimator.prototype.animateAnimation = function(animationname, animations, animbox, state, direction, animchar) {
*/

    if (direction && animations[direction]) {
        var diranim = animations[direction];
    ...
    if (animations[state]) {
        ...
        // todo: need way to store current animation by animation name, animation type and character part... 

        // { default, walk_left, legs }
        // { horns, walk_left, left_horn }
        // { tail, walk_left, tail }

        // need animationManager class?
