...
function startGame() {
    ...
    loadGame("dat/levels/level2.json",
             "dat/items.json",
             "dat/players.json", 
             "dat/characters.json", 
             "dat/animations.json");
...
function loadUI() {
    loadAjax("views/ui.html", function(data) {
        for (var i = 0; i < gameloop.stage.views.length; i++) gameloop.stage.views[i].setUI(new UI(data));
        loadDevTools();
    });
}
...
function loadDevTools() {
    loadAjax("views/ui_dev.html", function(data) {
        for (var i = 0; i < gameloop.stage.views.length; i++) gameloop.stage.views[i].setDevTools(new DevTools(data));
        resize();
        gameloop.showViews();
        running = true;
        run();
