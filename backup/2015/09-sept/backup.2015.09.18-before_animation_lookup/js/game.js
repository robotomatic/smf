...
function loadPlayers(file, charfile, animfile) {
    loadJSON(file, function(data){
        ...
        input.mapPlayerKeys(players[2], {80: "jump", 76: "left", 222: "right"});
...
function loadCharacters(file, animfile, players) {
    loadJSON(file, function(data){
        loadAnimations(animfile, new Characters().loadJson(data), players);
    }, function(data) {
        showError(data)
    });
}
...
function loadAnimations(file, chars, players) {
    loadJSON(file, function(data){
        ...
        for (var i = 0; i < players.length; i++) {
            chars[i].setAnimator(new CharacterAnimator(animations));
            chars[i].setRenderer(new CharacterRenderer());
            players[i].setCharacter(chars[i]);
        }
    }, function(data) {
        showError(data)
    });
...
function loadViews(players) {
    ...
    if (t.length == 1) {
        if (t[0].classList.contains('player-canvas')) {
            ...
            views = new Array(new PlayerView(id, player, 800, 400));
