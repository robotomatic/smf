...
ItemRenderer.prototype.drawItem = function(ctx, color, item, x, y, width, height, scale, drawdetails) {
    ...
    if (renderer) {
        if (renderer == "stars") this.starsrenderer.drawStars(ctx, item, x, y, width, height, scale, titem);
            ...
            this.wavesrenderer[item.name].drawWaves(ctx, titem.color, item, x, y, width, height, scale, titem);
    ...
    if (titem.parts) {
        ...
        if (item.draw != false) {
            this.drawItemParts(ctx, bitem.parts, x, y, width, height, color, scale);
        }
        if (item.parts) {
...
ItemRenderer.prototype.drawItemParts = function(ctx, parts, x, y, width, height, color, scale) { 
    var keys = Object.keys(parts);
    for (var i = 0; i < keys.length; i++) {
         var part = keys[i];
         this.drawItemPart(ctx, parts[part], x, y, width, height, color, scale);
    }
}

ItemRenderer.prototype.drawItemPart = function(ctx, part, x, y, width, height, color, scale) {
    if (part.height && part.width) this.drawPart(ctx, part, x, y, width, height, color, scale);
    if (part.parts) {
        color = (part.color) ? part.color : color;
        this.drawItemParts(ctx, part.parts, x, y, width, height, color, scale);
    }
}

ItemRenderer.prototype.drawPart = function(ctx, part, x, y, width, height, color, scale) {
