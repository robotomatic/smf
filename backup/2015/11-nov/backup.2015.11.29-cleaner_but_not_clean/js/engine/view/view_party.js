...
//    if (opp.length == 1 && opp.ready) testx = false;
    ...
    mbr = this.camera.getCameraBox(mbr, testx, amount);
...
//        if (this.drawleveldebug) this.renderLayerDebug(ctx, cmbrtrans, stage.level, layer, mbr_x, mbr_y, scale, dy);
        ...
        else this.renderLayerItems(delta, canvas, ctx, cmbrtrans, stage, layer, mbr_x, mbr_y, scale, dy);
    }
...
PartyView.prototype.renderLayerItems = function(delta, canvas, ctx, view, stage, layer, mbrx, mbry, scale, dy) {
    if (layer.items) for (var i = 0; i < layer.items.length; i++) this.renderLayerItem(delta, canvas, ctx, view, stage, layer, layer.items[i], mbrx, mbry, scale, dy, i);
    if (this.drawleveldebug) this.renderLayerDebug(ctx, view, stage, layer, mbrx, mbry, scale, dy);
}

PartyView.prototype.renderLayerItem = function(delta, canvas, ctx, view, stage, layer, item, mbrx, mbry, scale, dy, index) {
...
PartyView.prototype.renderPlayers = function(delta, ctx, stage, view, mbrx, mbry, scale, dy) {
    for (var i = 0; i < stage.players.length; i++) this.renderPlayer(delta, ctx, stage.players[i], view, mbrx, mbry, scale, dy, i);
}

PartyView.prototype.renderPlayer = function(delta, ctx, player, view, mbrx, mbry, scale, dy, index) {
    ...
    py = (py - mbry) * scale;
    py += dy;
    player.draw(ctx, view.x + px, view.y + py, player.width * scale, player.height * scale, scale);            
}
