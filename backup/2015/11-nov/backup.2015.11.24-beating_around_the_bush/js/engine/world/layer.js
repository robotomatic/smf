...
//    if (this.blend) ctx.globalCompositeOperation = this.blend;
    ...
    if (!cache || !this.itemcache.cacheItem(ctx, item, x, y, width, height, renderer, outline, outlinewidth, scale, this.drawdetails)) {
        if (renderer) {
            renderer.drawItem(ctx, item.color, item, x, y, width, height, this.lighten, outline, outlinewidth ? outlinewidth : this.outlinewidth, scale, this.drawdetails);
        } else {
...
Layer.prototype.collideItemPart = function(player, item, part) {
    ...
    if (part.collide) box.collide = part.collide;
