...
ItemCache.prototype.cacheItem = function(ctx, item, x, y, width, height, renderer, outline, outlinewidth, scale, details) {
    ...
    if (cacheitem) ctx.drawImage(cacheitem.canvas, x, y, width, height);
    else {
        ...
        if (renderer) renderer.drawItem(cacheitem.ctx, item.color, item, 0, 0, w, h, this.lighten, outline, outlinewidth ? outlinewidth : this.outlinewidth, 1, details);
        else {
